...
const searchDogName = (req, res) => {
  ...
  return Dog.findByName(req.query.name, (err, doc) => {
    ...
    // doc.age++;
    lastAddedDog.name = doc.name;
    lastAddedDog.breed = doc.breed;
    lastAddedDog.age = doc.age;
    updateLastDog(req, res);
    // if a match, send the match back
    return res.json({ name: lastAddedDog.name, breed: lastAddedDog.breed, age: lastAddedDog.age });
  });
};
